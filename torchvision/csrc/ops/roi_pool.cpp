...
std::tuple<at::Tensor, at::Tensor> roi_pool3d(
    const at::Tensor& input,
    const at::Tensor& rois,
    double spatial_scale,
    ...
    int64_t pooled_width) {
  static auto op = c10::Dispatcher::singleton()
                       .findSchemaOrThrow("torchvision::roi_pool3d", "")
                       .typed<decltype(roi_pool3d)>();
  return op.call(input, rois, spatial_scale, pooled_depth, pooled_height, pooled_width);
}
...
at::Tensor _roi_pool3d_backward(
    ...
    int64_t width) {
  static auto op = c10::Dispatcher::singleton()
                       .findSchemaOrThrow("torchvision::_roi_pool3d_backward", "")
                       .typed<decltype(_roi_pool3d_backward)>();
  return op.call(
      grad,
      rois,
      argmax,
...
TORCH_LIBRARY_FRAGMENT(torchvision, m) {
  ...
  m.def(TORCH_SELECTIVE_SCHEMA(
      "torchvision::roi_pool3d(Tensor input, Tensor rois, float spatial_scale,int pool_depth, int pooled_height, int pooled_width) -> (Tensor, Tensor)"));
  m.def(TORCH_SELECTIVE_SCHEMA(
      "torchvision::_roi_pool3d_backward(Tensor grad, Tensor rois, Tensor argmax, float spatial_scale, int pooled_depth, int pooled_height, int pooled_width, int batch_size, int channels, int depth, int height, int width) -> Tensor"));
}
