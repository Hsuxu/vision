...
__device__ inline bool devIoU(
  ...
  T up = max(a[0],b[0]), down = min(a[3],b[3]);
  T left = max(a[1], b[1]), right = min(a[4], b[4]);
  T top = max(a[2], b[2]), bottom = min(a[5], b[5]);
  T width = max(right - left, (T)0), height = max(bottom - top, (T)0), depth = max(up - down, (T)0);
  using acc_T = at::acc_type<T, /*is_cuda=*/true>;
  acc_T interS = (acc_T)width * height * depth;
  acc_T Sa = ((acc_T)a[3] - a[0]) * (a[4] - a[1])*(a[5] - a[2]);
  acc_T Sb = ((acc_T)b[3] - b[0]) * (b[4] - b[1])*(b[5] - b[2]);
  return (interS / (Sa + Sb - interS)) > threshold;
};

template <typename T>
__global__ void nms3d_kernel_impl(
    int n_boxes,
  ...
  if (threadIdx.x < col_size) {
    ...
    // block_boxes[threadIdx.x * 6 + 6] = dev_boxes[(threadsPerBlock * col_start + threadIdx.x) * 6 + 6];
  }
  __syncthreads();
...
at::Tensor nms3d_kernel(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      dets_sorted.scalar_type(), "nms3d_kernel", [&] {
        nms3d_kernel_impl<scalar_t><<<blocks, threads, 0, stream>>>(
            dets_num,
            iou_threshold,
