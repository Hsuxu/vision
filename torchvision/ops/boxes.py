...
def nms3d(boxes: Tensor, scores: Tensor, iou_threshold: float) -> Tensor:
    ...
    Args:
        boxes (Tensor[N, 6])): boxes to perform NMS on. They
            are expected to be in ``(z1, x1, y1, z2, x2, y2)`` format with ``0 <= x1 < x2`` and
            ``0 <= y1 < y2``.
        scores (Tensor[N]): scores for each one of the boxes
        iou_threshold (float): discards all overlapping boxes with IoU > iou_threshold
    ...
    """
    _assert_has_ops()
    return torch.ops.torchvision.nms3d(boxes, scores, iou_threshold)
...
def batched_nms3d(
    ...
    # from different classes do not overlap
        ...
        offsets = idxs.to(boxes) * (max_coordinate + torch.tensor(1).to(boxes))
        boxes_for_nms = boxes + offsets[:, None]
        keep = nms3d(boxes_for_nms, scores, iou_threshold)
        return keep
